...
@onready var px_boot_sim: PXBootSim = get_node_or_null("../PXBootSim") # NEW: For kernel boot simulation (M7)
...
func _process_next_instruction():
    ...
    match command:
        ...
        "PX_LOAD_KERNEL_PAYLOAD":
            _handle_px_load_kernel_payload(args)
        "PX_EXEC_KERNEL": # NEW: Handle PX_EXEC_KERNEL
            _handle_px_exec_kernel(args)
        _:
...
func _handle_px_scan_iso_dir(args: Array):
...
func _handle_px_load_kernel_payload(args: Array):
    if args.size() == 4 && args[2] == "TO=":
        var iso_path = _resolve_arg(args[0])
        var kernel_offset = _resolve_arg(args[1])
        var target_memory_key = _resolve_arg(args[3])
        
        _log("PXTalkVM: Simulating PX_LOAD_KERNEL_PAYLOAD from '%s' at offset %s to '%s'." % [iso_path, str(kernel_offset), target_memory_key])
        
        # Conceptual: In a real scenario, you'd read the actual kernel payload
        # from the ISO file at the given offset and store its binary data in memory.
        # For this simulation, we'll store a dummy "KERNEL_BINARY_DATA" string.
        
        _simulated_memory[target_memory_key] = "KERNEL_BINARY_DATA_LOADED"
        _log("PXTalkVM: Simulated kernel payload loaded to '%s'." % target_memory_key)
    else:
        _log("PXTalkVM: Error: PX_LOAD_KERNEL_PAYLOAD requires iso_path kernel_offset TO= target_memory_key.")
        _halt_vm("INVALID_LOAD_KERNEL_PAYLOAD_SYNTAX")

func _handle_px_exec_kernel(args: Array): # NEW: PX_EXEC_KERNEL handler
    if args.size() == 1:
        var kernel_payload_memory_key = _resolve_arg(args[0])
        var kernel_data = _simulated_memory.get(kernel_payload_memory_key, null)
        
        if kernel_data != null:
            _log("PXTalkVM: Simulating PX_EXEC_KERNEL from memory key '%s'." % kernel_payload_memory_key)
            # This is the point where the simulated PXCPU would take over.
            # For now, it will trigger PXBootSim.gd for visual kernel boot simulation.
            
            if px_boot_sim:
                _log("PXTalkVM: Triggering PXBootSim for kernel boot visualization.")
                # Provide some dummy kernel boot messages for PXBootSim
                var simulated_kernel_logs = [
                    "[KERNEL] Initializing CPU...",
                    "[KERNEL] Detecting memory...",
                    "[KERNEL] Loading drivers...",
                    "[KERNEL] Mounting root filesystem...",
                    "[KERNEL] Starting init process...",
                    "[KERNEL] Boot complete."
                ]
                px_boot_sim.start_simulation(simulated_kernel_logs)
            else:
                _log("PXTalkVM: Warning: PXBootSim not available for kernel visualization.")
            
            _log("PXTalkVM: Conceptual kernel execution initiated. Transferring control...")
            _halt_vm("KERNEL_EXECUTING") # Halt VM as kernel takes over conceptual control
        else:
            _log("PXTalkVM: Error: PX_EXEC_KERNEL: Kernel payload not found at '%s'." % kernel_payload_memory_key)
            _halt_vm("KERNEL_PAYLOAD_MISSING")
    else:
        _log("PXTalkVM: Error: PX_EXEC_KERNEL requires kernel_payload_memory_key.")
        _halt_vm("INVALID_EXEC_KERNEL_SYNTAX")

...
# managing simulated memory, logs, and interaction with UI components.
...
func boot_pxboot_file(boot_file_path: String):
    ...
    # Parse boot_content to get entry point and interpreter path (conceptual)
    # For this scaffold, we'll directly load and call the BIOS interpreter.
...
func _process_next_instruction():
    ...
    match command:
        ...
        "PX_CALL_FUNCTION": # NEW: Handle PX_CALL_FUNCTION
            _handle_px_call_function(args)
        "PX_GET_ARG": # NEW: Handle PX_GET_ARG
            _handle_px_get_arg(args)
        "PX_SET_VAR": # NEW: Handle PX_SET_VAR
            _handle_px_set_var(args)
        "PX_READ_MEM":
            _handle_px_read_mem(args)
        "PX_IF_EQ": # NEW: Handle PX_IF_EQ
            _handle_px_if_eq(args)
        "PX_ELSE": # NEW: Handle PX_ELSE
            _handle_px_else()
        "PX_ELSE_IF_STARTS_WITH": # NEW: Handle PX_ELSE_IF_STARTS_WITH
            _handle_px_else_if_starts_with(args)
        "PX_LOOP": # NEW: Handle PX_LOOP
            _handle_px_loop()
        "PX_BREAK_LOOP": # NEW: Handle PX_BREAK_LOOP
            _handle_px_break_loop()
        "PX_READ_INPUT": # NEW: Handle PX_READ_INPUT
            _handle_px_read_input(args)
        "PX_GET_SUBSTRING": # NEW: Handle PX_GET_SUBSTRING
            _handle_px_get_substring(args)
        "PX_READ_MEM_RANGE": # NEW: Handle PX_READ_MEM_RANGE
            _handle_px_read_mem_range(args)
        "PX_GET_MEM_STATE": # NEW: Handle PX_GET_MEM_STATE
            _handle_px_get_mem_state(args)
        "PX_SET_MEM_STATE": # NEW: Handle PX_SET_MEM_STATE
            _handle_px_set_mem_state(args)
        "PX_RETURN": # NEW: Handle PX_RETURN
            _return_from_call()
        _:
...
func _reset_vm_state():
    ...
    _waiting_for_input = false
    _input_buffer = ""
    _last_instruction_time = 0.0 # Reset timer for process loop

# --- PXTalk Instruction Handlers ---

var _last_instruction_time := 0.0 # For simulating typing speed
...
func _handle_px_print_str(args: Array):
    ...
    if px_framebuffer:
        # Conceptual: PXFramebuffer would need a draw_string method
        # px_framebuffer.draw_string(message, Color.WHITE, Vector2(0, _simulated_memory.get("display_y_pos", 0)))
        # _simulated_memory["display_y_pos"] = _simulated_memory.get("display_y_pos", 0) + 10
        pass # For now, just log to terminal
    if px_log_terminal:
...
func _handle_px_clear_screen(args: Array):
    if args.size() == 1:
        ...
        # Conceptual: px_framebuffer.clear_screen(Color.from_html(color_hex))
        _log("PXTalkVM: Screen cleared with color: " + str(color_hex))
    else:
...
func _handle_px_load_file(args: Array):
    if args.size() >= 2 and args[1] == "AS_MODULE":
        ...
        # Conceptual: In a real VM, this would load the module's code for later execution.
...
func _handle_px_call_function(args: Array): # NEW: PX_CALL_FUNCTION handler
    if args.size() >= 1:
        var function_name = _resolve_arg(args[0])
        var function_args = args.slice(1) # Remaining args are for the called function
        # This calls a function within the *current* module
...
func _handle_px_get_arg(args: Array): # NEW: PX_GET_ARG handler
    if args.size() == 2:
...
func _handle_px_set_var(args: Array): # NEW: PX_SET_VAR handler
    if args.size() == 2:
        var var_name = _resolve_arg(args[0])
        var value = _resolve_arg(args[1])
        _registers[var_name] = value # Treat as a register/variable
...
func _handle_px_read_mem(args: Array):
    if args.size() == 2:
        var address = _resolve_arg(args[0])
        var target_register = _resolve_arg(args[1])
        _registers[target_register] = _simulated_memory.get(str(address), null) # Read from conceptual memory
...
func _handle_px_if_eq(args: Array): # NEW: PX_IF_EQ handler
    if args.size() == 3:
        var val1 = _resolve_arg(args[0])
        var val2 = _resolve_arg(args[1])
        var jump_target_label = args[2] # This is a label, not an IP. Needs to find label.
...
func _handle_px_else(): # NEW: PX_ELSE handler (conceptual for IF/ELSE blocks)
    # This assumes _handle_px_if_eq or similar would have skipped to here if false.
    # For a real interpreter, you'd need to manage conditional jumps.
    _log("PXTalkVM: PX_ELSE encountered. Executing else block.")
    pass

func _handle_px_else_if_starts_with(args: Array): # NEW: PX_ELSE_IF_STARTS_WITH handler
    if args.size() == 3:
...
func _handle_px_loop(): # NEW: PX_LOOP handler
    # Push current loop state (start IP) to a separate loop stack
    _call_stack.back()["loop_start_ip"] = _instruction_pointer - 1 # IP is already incremented
    _log("PXTalkVM: PX_LOOP entered. Loop start IP: " + str(_instruction_pointer - 1))

func _handle_px_break_loop(): # NEW: PX_BREAK_LOOP handler
    # Find the nearest loop on the stack and jump past its end
    var loop_start_ip = -1
    ...
    if loop_start_ip != -1:
        ...
        if loop_end_ip != -1:
            _instruction_pointer = loop_end_ip + 1 # Jump past the loop
...
func _find_loop_end_ip(loop_start_ip: int) -> int:
    """Helper to find the instruction pointer of the end of a loop block."""
    # This is a simplified search. A real interpreter would parse block structures.
    # For now, assume a loop ends when the function ends or a PX_RETURN is hit
    # or a specific 'PX_END_LOOP' instruction (not yet defined).
    # This needs to be smarter for nested loops.
    # For now, it will just return the end of the current function if no explicit end.
    var current_depth = 0
    for i in range(loop_start_ip, _current_instructions.size()):
        ...
        if instruction == "PX_LOOP":
            current_depth += 1
        # Conceptual: if we had PX_END_LOOP or similar
        # elif instruction == "PX_END_LOOP":
        #    current_depth -= 1
        #    if current_depth == 0:
        #        return i
        elif instruction == "PX_RETURN": # Loop ends with function return
            return i
    return _current_instructions.size() - 1 # Assume loop goes to end of function if no explicit end

func _handle_px_read_input(args: Array): # NEW: PX_READ_INPUT handler
    if args.size() == 2 && args[0] == "->":
        ...
        # The _process loop will pause until submit_input is called.
...
func _handle_px_get_substring(args: Array): # NEW: PX_GET_SUBSTRING handler
...
func _handle_px_read_mem_range(args: Array): # NEW: PX_READ_MEM_RANGE handler
...
func _handle_px_get_mem_state(args: Array): # NEW: PX_GET_MEM_STATE handler
...
func _handle_px_set_mem_state(args: Array): # NEW: PX_SET_MEM_STATE handler
    if args.size() == 2:
        var key = _resolve_arg(args[0])
        var value = _resolve_arg(args[1])
...
func _get_current_module_content() -> String:
    ...
    # If it's the initial boot module (boot.pxboot), its content might not be in LOADED_MODULE_
    # This needs a more robust way to get current module content.
    # For now, it will assume interpreter/kernel/shell are loaded via LOAD_FILE AS_MODULE.

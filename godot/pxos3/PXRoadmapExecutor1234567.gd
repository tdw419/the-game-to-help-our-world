...
#
# UPDATED: Now includes the 'WRITE_FILE_AUTO' command for automatic
# file generation and writing to PXFS using PXFSWriter.write_file_auto().
...
func _ready():
    ...
    if not px_scroll_log or not px_condition_checker or not px_roadmap_memory or \
       ...
       not px_digest_exporter or not px_fs_writer:

...
# ✅ Generates a red-diagonal pxboot.png for testing
...
# ✅ Load pxboot image into the runtime texture
func load_pxboot():
    px_image = pxboot_texture.get_image()
    px_texture = ImageTexture.create_from_image(px_image)

# ✅ Create a display sprite for the framebuffer
func setup_display():
...
# ✅ Main update loop – processes each frame
func _process(_delta):
    px_talk_tick()
    px_texture.update(px_image)

# ✅ Iterate every pixel, run PXTalk logic
func px_talk_tick():
...
# ✅ Example prototype: toggle pixels based on red channel
func run_px_instruction(x: int, y: int, color: Color):
    if color.r > 0.5:
        px_image.set_pixel(x, y, Color(0, 0, 0))  # Set black
    elif color.r == 0:
        px_image.set_pixel(x, y, Color(1, 0, 0))  # Set red

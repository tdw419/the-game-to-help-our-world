...
#
# UPDATED: Now includes the 'LOAD_ROADMAP_FROM_FS' command to load and
# execute roadmaps directly from the Pixel Native File System (PXFS).
...
func _ready():
    ...
    if not px_scroll_log or not px_condition_checker or not px_roadmap_memory or \
       ...
       not px_digest_exporter or not px_fs_writer or not px_fs_reader: # PXFSReader is now essential

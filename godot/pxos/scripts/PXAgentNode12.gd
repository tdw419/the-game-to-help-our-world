...
#
# UPDATED: Now defines a class_name and common methods for agent commands.
...
func _process(delta):
    ...
    if display_image and display_image.is_locked():
        ...
        if frame_counter >= AGENT_CYCLE_FREQUENCY:
            ...
            # print("\n--- PXAgentNode '", agent_id, "' Cycle Start ---") # Too chatty
            observe_region()
...
func observe_region():
    ...
    if message_read:
        ...
        # _log_agent_activity("Detected glyph message: '" + message_read + "'") # Too chatty
    else:
    ...
    if detected_opcode_color.is_equal_approx(Color(1.0, 0.0, 0.0)): # Assuming PXL_HALT
...
func update_local_state():
    ...
    # Base implementation (can be extended/overridden)
    pass # _log_agent_activity("Local state updated. Mood: " + local_state["mood"]) # Too chatty

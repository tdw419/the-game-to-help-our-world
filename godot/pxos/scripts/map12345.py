import os
from PIL import Image, ImageDraw, ImageOps, ImageFont # NEW: Import ImageFont
import math
import random
import sys
import json
...
MAP_PY_VERSION = "2.3.0" # Updated version for RRE Dev Env with PXFS_RRE_v1.0 Roadmap
...
# --- PXFS_RRE_v1.0 Phase 1: Structural Upgrades (Hardcoded for this RRE execution) ---
# These values will be used when drawing the PXFS if include_pxfs is True
PXFS_EXPANDED_BLOCK_WIDTH = 12 # Smaller blocks to fit more files (20x10 grid)
PXFS_EXPANDED_BLOCK_HEIGHT = 12
PXFS_EXPANDED_FILES_PER_ROW = 20 # 20 files per row
PXFS_PADDING = 2 # 2px padding for block boundary clarity
...
def draw_filesystem_region(draw: ImageDraw.ImageDraw, files: list, origin: tuple, block_size: tuple):
    ...
    for i, file in enumerate(files):
        row = i // PXFS_EXPANDED_FILES_PER_ROW # Use expanded files per row
        col = i % PXFS_EXPANDED_FILES_PER_ROW # Use expanded files per row
        x0 = origin[0] + col * (block_w + PXFS_PADDING) # Add padding
        y0 = origin[1] + row * (block_h + PXFS_PADDING) # Add padding
        x1 = x0 + block_w
        y1 = y0 + block_h
...
def generate_pixel_substrate_map(
    ...
    include_roadmap_scroll: bool = True # NEW: Toggle for roadmap scroll
):
    ...
    if external_elevation_path:
        try:
            ...
            print(f"  ERROR: External elevation file '{external_elev_path}' not found. Using procedural elevation.")
            external_elev_img = None
        except Exception as e:
    ...
    # Draw PXFS if enabled
    if include_pxfs:
        # Prepare file list from PXMemory['godot_rre']['modules']
        pxfs_files_to_draw = []
        for module_name, data in PXMemory['godot_rre']['modules'].items():
            # Example mapping of module metadata to PXFS file dict
            file_size = data['metadata'].get('size', len(data['source'])) # Use source length if size not in metadata
            file_type_color = (0, 0, 255) if module_name.endswith(".gd") else (100, 100, 100) # Blue for GDScript
            flags_color = (0, 255, 0) if data['status'] == 'mutated' else (50, 50, 50) # Green if mutated
            hash_val = sum(ord(c) for c in data['source']) % 256 # Simple conceptual hash
            hash_color = (hash_val, hash_val, hash_val) # Grayscale hash
            
            pxfs_files_to_draw.append({
                "name": module_name + ".gd",
                "size": file_size,
                "type_color": file_type_color,
                "flags_color": flags_color,
                "hash_color": hash_color,
                "color": (220, 180, 240) if data['status'] == 'mutated' else (200, 200, 200) # Lighter if mutated
            })
        
        print(f"  Drawing {len(pxfs_files_to_draw)} PXFS file blocks.")
        # Use expanded PXFS constants for drawing
        draw_filesystem_region(draw, pxfs_files_to_draw, origin=FS_ROOT_ORIGIN, 
                               block_size=(PXFS_EXPANDED_BLOCK_WIDTH, PXFS_EXPANDED_BLOCK_HEIGHT))

    # NEW: Draw Roadmap Scroll if enabled
    if include_roadmap_scroll:
        print("  Drawing PXFS_RRE_v1.0 roadmap scroll.")
        render_roadmap_scroll(draw, PXFS_RRE_V1_0_ROADMAP, 
                              origin=ROADMAP_SCROLL_REGION_ORIGIN, 
                              width=ROADMAP_SCROLL_REGION_WIDTH, 
                              height=ROADMAP_SCROLL_REGION_HEIGHT)
...
""")
        ...
        generate_pixel_substrate_map(
            ...
            include_roadmap_scroll=True # NEW: Enable roadmap scroll drawing
        )

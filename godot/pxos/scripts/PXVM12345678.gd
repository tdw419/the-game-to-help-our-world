...
# enhanced filename flexibility (Phase 5), and handoff to KernelSim (Phase 6).
...
func _execute_pxtalk_script():
	...
	# --- Phase 5: ISO Enhancements (Joliet placeholder) ---
	# In a full implementation, you would scan for SVDs (Joliet/Rock Ridge) here
	# if the PVD doesn't contain the necessary info or if you want extended features.
	# For now, we'll assume standard ISO9660 parsing from PVD is sufficient.
	_execute_px_log("[PXBIOS] Checking for Joliet/Rock Ridge extensions (conceptual)...")
	...
	# Locate kernel + initrd using full ISO path with fuzzy matching (Phase 5)
	...
	if pxram["kernel_data"] and pxram["initrd_data"]:
		...
		_execute_px_log("[PXBIOS] Phase 2, 3, 4 & 5 Complete.")
		# Handoff to KernelSim (Phase 6)
...
# --- ISO Path Navigator (Phase 5: Fuzzy Matching) ---
...
# --- Kernel Simulation Handoff (Phase 6) ---
func _execute_px_exec_kernel():
	_execute_px_log("[PXBIOS] Handoff to Kernel Simulation Layer (KernelSim.gd)...")
	yield(get_tree().create_timer(1.0), "timeout")
	
	# Load the KernelSim scene
	var kernel_sim_scene = load("res://KernelSim.tscn").instantiate()
	get_tree().get_root().add_child(kernel_sim_scene)
	
	# Pass kernel/initrd data to the KernelSim scene
	if kernel_sim_scene.has_method("set_kernel_data"):
		kernel_sim_scene.set_kernel_data(pxram["kernel_data"])
	if kernel_sim_scene.has_method("set_initrd_data"):
		kernel_sim_scene.set_initrd_data(pxram["initrd_data"])
	
	# Hide the current PXBootSim scene (assuming it's the root's child)
	# You might need to adjust this depending on your scene tree structure
	if get_parent():
		get_parent().hide()

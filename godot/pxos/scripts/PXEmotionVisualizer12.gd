...
# Threshold for drawing an outline/glow for peak emotions (0.0 to 1.0)
@export var peak_emotion_threshold: float = 0.85
@export var peak_outline_color: Color = Color(1.0, 1.0, 1.0, 0.6) # Semi-transparent white
@export var peak_outline_width: float = 2.0
...
func _draw():
    ...
    var percent_font_size = 5 * display_scale.x # Even smaller for percentage
    ...
    for emotion_name in emotion_names:
        ...
        # --- Add Outline/Glow for Peak Emotions ---
        if emotion_level >= peak_emotion_threshold:
            # Draw an outline around the filled part of the bar
            draw_rect(Rect2(bar_fill_rect_pos, bar_fill_rect_size), peak_outline_color, false, peak_outline_width * display_scale.x)

        # --- Add Numeric % Label ---
        var percent_text = str(round(emotion_level * 100)) + "%"
        # Position to the right of the bar, aligned with the bar's top
        var percent_pos = (Vector2(bar_pos_x + bar_width_max + 4, bar_pos_y + font_size + 1) * display_scale) + display_global_pos
        draw_string(font, percent_pos, percent_text, H_ALIGNMENT_LEFT, -1, percent_font_size, text_color)

...
# This should be a region where other modules can write a new roadmap (e.g., PXRoadmapGenerator).
@export var watched_roadmap_region: Rect2 = Rect2(0, 0, 40, 60) # Example: Top-left area for incoming roadmaps
...
func _ready():
    ...
    # Clear the watched region at startup
    px_ztxt_memory._clear_region(watched_roadmap_region)
...
func _scan_for_new_roadmap():
    ...
    if current_hash != _last_read_roadmap_hash and not current_roadmap_ztxt.is_empty():
        ...
        # Clear the region to consume the roadmap (optional, but good for one-shot triggers)
        ...
        if not roadmap_steps.is_empty():
            ...
            px_roadmap_executor.execute_roadmap(roadmap_steps) # Assuming execute_roadmap takes Array[String]
...
func _parse_roadmap_content(ztxt_content: String) -> Array[String]:
    ...
    Assumes each line is a step. Can be extended to handle headers/footers.
    """
    var lines = ztxt_content.split("\n", false)
    var parsed_roadmap: Array[String] = []
    for line in lines:
        var trimmed_line = line.strip_edges()
        if not trimmed_line.is_empty() and not trimmed_line.begins_with("#"): # Ignore comments

...
MAP_PY_VERSION = "2.2.0" # Updated version for RRE Dev Env with PXFS
...
FS_BLOCK_WIDTH = 16
FS_BLOCK_HEIGHT = 16
FILES_PER_ROW = 5 # Number of file blocks per row in the FS display
...
def draw_filesystem_region(draw: ImageDraw.ImageDraw, files: list, origin: tuple, block_size: tuple):
    ...
    for i, file in enumerate(files):
        row = i // FILES_PER_ROW
        col = i % FILES_PER_ROW
        x0 = origin[0] + col * block_w
        y0 = origin[1] + row * block_h
        x1 = x0 + block_w
        y1 = y0 + block_h
...
def generate_pixel_substrate_map(
    ...
    include_pxfs: bool = True # NEW: Toggle for PXFS
):
    ...
    if include_pxfs:
        ...
        draw_filesystem_region(draw, pxfs_files_to_draw, origin=FS_ROOT_ORIGIN, block_size=(FS_BLOCK_WIDTH, FS_BLOCK_HEIGHT))
...
""")
        ...
        generate_pixel_substrate_map(
            ...
            include_pxfs=True # NEW: Enable PXFS drawing
        )

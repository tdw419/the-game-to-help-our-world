...
# App Manager to load and manage apps
class AppManager:
    ...
    def run_active_app(self):
        if self.active_app:
            try:
                ...
                print(f"Error running app {self.active_app.name}: {e}")
    ...
    def render_active_app(self, screen):
        if self.active_app:
            try:
                ...
                print(f"Error rendering app {self.active_app.name}: {e}")

# Initialize PXOSInterface and AppManager
pxos = PXOSInterface()
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Example: Press '1' to activate 'test_app'

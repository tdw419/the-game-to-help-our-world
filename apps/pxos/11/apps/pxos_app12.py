...
class MiniRT:
    ...
    def exec_code(self, name):
        ...
        if code:
            try:
                tree = ast.parse(code)
                if all(isinstance(node, (ast.FunctionDef, ast.ClassDef)) for node in tree.body):
                    exec(code, globals())
                    return f"Executed: {name}"
                return "Only function/class definitions allowed"
...
# PXBot class with pixel programming tools
class PXBot:
...
# PXBotGUI class for Tkinter interface
class PXBotGUI:
    def __init__(self):
        self.vfs = MiniVFS()
        self.runtime = MiniRT()
